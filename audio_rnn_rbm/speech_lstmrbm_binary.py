...
def test_lstmrbm(X, batch_size=100, num_epochs=1000):
    model = LstmRbm(X[0].shape[1], n_hidden=600, n_hidden_recurrent=400)
...
def to_int(x):
    return int("".join([str(int(xi)) for xi in x]), 2)
...
if __name__ == '__main__':
    ...
    n_lsf_clusters = 128
    n_lsf_binary = int(np.ceil(np.log2(n_lsf_clusters)))
    lsf_tf = KMeans(n_clusters=n_lsf_clusters, random_state=1999)
    lsf_tf.fit(lsf_kmeans_samples)
    n_gain_clusters = 128
    n_gain_binary = int(np.ceil(np.log2(n_gain_clusters)))
    ...
    for n, d in enumerate(train_x):
        ...
        lsf_binary = np.zeros((length, n_lsf_binary)).astype(
            theano.config.floatX)
        gain_labels = gain_tf.predict(gain)
        gain_binary = np.zeros((length, n_gain_binary)).astype(
            theano.config.floatX)

        for i in np.arange(length):
            lsf_idx = np.where(to_bin(lsf_labels[i], n_lsf_binary))[0]
            gain_idx = np.where(to_bin(gain_labels[i], n_gain_binary))[0]
            lsf_binary[i, lsf_idx] = 1.
            gain_binary[i, gain_idx] = 1.
        binary = np.concatenate((lsf_binary, gain_binary), axis=1)
        X.append(binary)


    def reconstruct(codebook, binary, n_features):
        arr = np.zeros((len(binary), n_features))
    ...
    for i in [0, 20, 40, 60, 80, 100]:
        lsf = reconstruct(lsf_tf.cluster_centers_, X[i][:, :n_lsf_binary],
                          n_features)
        a = lsf_to_lpc(lsf)
        gain = reconstruct(gain_tf.cluster_centers_, X[i][:, -n_gain_binary:],
                           1)
    ...
    for i in range(n_samples_to_gen):
        g = model.generate()
        lsf = reconstruct(lsf_tf.cluster_centers_, g[:, :n_lsf_binary],
                          n_features)
        a = lsf_to_lpc(lsf)
        gain = reconstruct(gain_tf.cluster_centers_, g[:, -n_gain_binary:],
                           1)
        X_s = lpc_synthesis(a, gain, window_step=window_step)
