...
def build_rbm(v, W, bv, bh, k):
    ...
    v : Theano vector or matrix
      If a 3D, multiple chains will be run in parallel (batch).
    W : Theano matrix
      Weight matrices of the RBM.
    bv : Theano vector
      Visible bias vectors of the RBM.
    bh : Theano vector
...
def build_lstm_softmax_rbm(n_classes, n_visible, n_hidden, n_hidden_recurrent):
    ...
    W, = make_weights(n_visible, [n_hidden], random_state,
...
class LSTMSoftmaxRBM:
    '''
    Simple class to train an RNN-RBM to generate sample
    sequences.
    '''
    def __init__(self, n_classes, n_visible, n_hidden=150,
                 n_hidden_recurrent=100, lr=0.0001):
...
if __name__ == '__main__':
    ...
    key_range, dt, dataset = fetch_nottingham()
    # make one hot to test softmax part...
    dataset = [dense_to_one_hot(d, n_classes=2) for d in dataset]
    if not os.path.exists('saved.pkl'):
        model = LSTMSoftmaxRBM(n_classes=2, n_visible=88)
