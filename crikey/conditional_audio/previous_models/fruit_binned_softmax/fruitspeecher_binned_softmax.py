...
if __name__ == "__main__":
    ...
    n_softmax = 10
    n_softmax_proj = n_softmax * input_dim
    ...
    def sample_step(x_tm1, h1_tm1, h2_tm1, h3_tm1, k_tm1, w_tm1, ctx):
        ...
        out_t = h1_t.dot(h1_to_outs) + h2_t.dot(h2_to_outs) + h3_t.dot(
            h3_to_outs)
        pre_pred_t = out_t.dot(softmax_proj) + softmax_b
        shp = pre_pred_t.shape
        pre_pred_t = pre_pred_t.reshape((-1, n_softmax))
        pred_t = softmax(pre_pred_t)

        theano.printing.Print("pred_t.shape")(pred_t.shape)
        s_t = sample_softmax(pred_t, srng)
        theano.printing.Print("s_t.shape")(s_t.shape)
        x_t = s_t.reshape((shp[0], -1))
    ...
    pre_pred = outs.dot(softmax_proj) + softmax_b
    pre_pred = pre_pred.reshape((-1, n_softmax))
    ...
    shp = target.shape
    target = target.ravel()
    target = theano_one_hot(target, n_classes=n_softmax)
    target = target.reshape((-1, n_softmax))
    theano.printing.Print("target.shape")(target.shape)

    cost = categorical_crossentropy(pred, target)
    cost = cost.reshape((shp[0], shp[1], shp[2]))
