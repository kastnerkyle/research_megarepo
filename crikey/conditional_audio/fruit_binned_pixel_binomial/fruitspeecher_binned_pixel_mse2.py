...
if __name__ == "__main__":
    ...
    def sample_step(x_tm1, h1_tm1, h2_tm1, h3_tm1, k_tm1, w_tm1, ctx):
        ...
        j_t = concatenate((x_tm1_shuf, vinp_t), axis=-1)
        vinp_h1_t, vinpgate_h1_t = outs_to_v_h1.proj(j_t)

        def sample_out_step(vinp_h1_f, vinpgate_h1_f, v_h1_tm1):
            v_h1_f = v_cell1.step(vinp_h1_f, vinpgate_h1_f, v_h1_tm1)
            return v_h1_f

        pre_pred_t, isupdates = theano.scan(
            fn=sample_out_step,
        ...
        # MSE
        pred_t = pre_pred_t.dot(pred_proj) + pred_b
        x_t = pred_t
    ...
    sampled = sampled.dimshuffle(2, 1, 0)
    ...
    shp = pre_pred.shape
    # since we reshaped by merging 2 * minibatch size have to invert it the same
