...
full_sines = full_sines[:, :, None].astype(theano.config.floatX)
mag = np.linspace(0, np.max(full_sines), n_timesteps // 2)
mag = np.concatenate((mag, mag[::-1]))
mag = np.concatenate(20 * [mag])
mag = mag[:len(full_sines), None, None]
full_sines = full_sines * mag
